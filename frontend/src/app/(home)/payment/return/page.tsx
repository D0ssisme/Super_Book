...
const PaymentReturnContent = () => {
  ...
  const isPaid = status === "PAID" || resultCode === "0";
  const resolvedOrderCode = orderCode || momoOrderId || "";
  ...
  useEffect(() => {
    const syncPaidStatus = async () => {
      if (!isPaid || !resolvedOrderCode) return;
      try {
        const order: Order = await getOrderByOrderCode(resolvedOrderCode);
        if (order?._id && order.paymentStatus !== "paid") {
          await confirmPayment(order._id);
        }
      } catch (error) {
        console.error("Sync payment status failed", error);
      }
    };

    syncPaidStatus();
  }, [isPaid, resolvedOrderCode]);
  ...
  const handleRouterDetails = async () => {
    const res: Order = await getOrderByOrderCode(resolvedOrderCode);
    console.log(res);
