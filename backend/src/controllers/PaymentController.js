...
export async function createPayment(req, res) {
  return res.status(501).json({ok: false, message: 'PayOS functionality is currently disabled'});
}
export async function webhookController(req, res){
  return res.status(501).json({message: 'PayOS webhook is currently disabled'});
}
export async function cancelPayment(req, res){
  return res.status(501).json({message: 'PayOS cancel is currently disabled'});
}